...
class CapitalTab(ttk.Frame):
    def __init__(self, parent):
        ...
        def left_callback(var, index, mode):
            try:
                ...
                self.left_clock = AnalogClock(
                    self.left,
                    str_to_capital(self.selected_capital_one.get(), capitals),
                    10,
                )
        ...
        def right_callback(var, index, mode):
            try:
                ...
                self.right_clock = AnalogClock(
                    self.right,
                    str_to_capital(self.selected_capital_two.get(), capitals),
                    10,
                )
        ...
        self.country_dropdown_one = CountryDropdown(
            self.left, self.selected_capital_one
        )
        self.country_dropdown_one.pack()

        self.country_dropdown_two = CountryDropdown(
            self.right, self.selected_capital_two
        )
        ...
        self.info_text = tk.StringVar(
            value="Te miasta różnią się o: "
            + str(
                get_time_diff(self.current_time_one.get(), self.current_time_two.get())
            )
        )
